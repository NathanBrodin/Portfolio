...
So I refactored it with a monorepo structure to keep everything perfect using the better-t-stack template, and decided to rebuild it entirely from the ground up to fetch the data directly from my accounts, using the "Enable Banking" PSD2 provider, but after a month building on it, realized that the data is unusable, like transactions are not categorized and contain little information, and the worst was that you have to first connect your accounts to the Enable Banking admin dashboard and then you could use it in my app, but this means I can never publish the app.
