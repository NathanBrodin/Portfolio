...
### What it is, and where I came in
...
Around 223 out of ~1,325 commits in the repo.
...
Deployed GitHub Actions self-hosted runners with Docker Compose: 12 containers of `myoung34/github-runner`, registered at the organization level so every repo in the org can use them. Then I switched my pipelines from `runs-on: ubuntu-latest` to `runs-on: self-hosted` and shared the setup with the team.
